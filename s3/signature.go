...
func (s3 *S3) GenerateSignature(params GenerateSignatureParams) Signature {
	...
	var cred = fmt.Sprintf("%s/%s/%s/s3/aws4_request", s3.config.AccessKey, formattedShortTime, s3.config.Region)
	...
	// Generate signature
	h1 := makeHmac([]byte("AWS4"+s3.config.SecretKey), []byte(date[:8]))
	h2 := makeHmac(h1, []byte(s3.config.Region))
